...
% end
    ...
    [b,a,~] = uigetfile('D:\MIMIC II WAVEFORM DATABASE\Test');
    reply = fullfile(a,b);
    ...
    reply = uigetdir('D:\MIMIC II WAVEFORM DATABASE\Test');
    ...
    fileList = dir(fullfile(reply, '*.txt')); %get list of files
    ...
    disp([reply ' contains ' num2str(length(fpaths)) ' .txt files.'])
    
    %choose output file name to save hrv results
    [outPath2,outPath1,~] = uiputfile('D:\MIMIC II WAVEFORM DATABASE\Test\*.csv');
    outPath = fullfile(outPath1,outPath2);
    if isempty(outPath)
        error('Please choose valid input dir path!')
        return;
    end
